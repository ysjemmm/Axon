...
export interface AgentPolicy {
  ...
  /** 投降前允许的"深度复盘"次数（重量层：注入更强的复盘引导，保留完整上下文） */
...
/**
 * 深度复盘引导（重量层）：反思后仍反复失败时，注入更强的复盘要求。
 * 不压缩上下文--失败原文（哪条命令报了什么错、哪个文件改了几次）恰恰是避免重蹈覆辙的依据。
  */
export function buildSummaryRestartPrompt(stuck: StuckTarget | null): string {
  const where = stuck?.path ? `（尤其是之前卡住的「${stuck.path}」）` : "";
  return (
    `⚠️ 你在反思后仍然反复失败。上下文中的完整历史保留着--不要忽略它，那是你避免重蹈覆辙的依据。\n` +
    `现在请做一次【深度复盘】，逐条对照上下文中的真实工具返回（不要凭印象）:\n` +
    `1. 列出你试过的每一种【不同】做法，每一种引用真实的报错/返回值说明为什么失败\n` +
    `2. 根因分析：是路径不对？参数不匹配？编译产物与源码不同步？还是这条路本身走不通？\n` +
    `3. 基于根因，选一条【明确不同】且【尚未尝试】的路径${where}\n` +
    `4. 如果所有可行路径都已穷尽，如实告诉用户原因和建议，不要再空转\n` +
    `硬性要求：禁止用与之前相同或本质相似的参数/方式调用同一个工具。`
  );
}
