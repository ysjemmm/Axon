...
export function applyEventToSubAgent(sub: SubAgentSegment, event: WsMessage): SubAgentSegment {
  ...
  if (type === "tool_call") {
      ...
      for (let i = inner.length - 1; i >= 0; i--) {
        ...
        if (seg.type === "tool" && seg.status === "pending" && (seg.id === eventId || (!seg.boundId && seg.name === event.name))) {
          inner[i] = {
            ...
            query: (event.name === "search" || event.name === "list_dir") ? exploreDisplayText(event.name || "", args, seg.query) : seg.query,
          };
          return { ...sub, inner };
        }
      }
    ...
    const toolSeg: ToolSegment = {
      ...
      query: (event.name === "search" || event.name === "list_dir") ? exploreDisplayText(event.name || "", args) : undefined,
    };
    inner.push(toolSeg);
    return { ...sub, inner };
  }
  ...
  if (type === "tool_result") {
    ...
    if (matchIdx >= 0) {
      ...
      else if (isExplore) finalDesc = exploreDisplayText(seg.name, ((event as any).args || seg.args) as Record<string, unknown>, seg.query);
      else if (fileName) {
